...
app.post('/api/analyze-menu', async (req, res) => {
    try {
        ...
        // Enrich dishes with pronunciations and allergens (single round-trip each)
        try {
            const uniqueNames = Array.from(new Set(allDishes.map(d => d && d.original_name).filter(Boolean)));
            if (uniqueNames.length > 0) {
                const [pronunciations, allergens] = await Promise.all([
                ...
                allDishes.forEach(d => {
                    ...
                    d.allergens = (allergens && allergens[d.original_name]) || '';
                });
            }
        } catch (e) {
...
// Helper: infer possible allergens for dishes using name + description
async function getAllergensForItems(items) {
    try {
        ...
        const prompt = `For each dish below, list likely food allergens based on the name and description. Use plain words like milk, egg, wheat/gluten, soy, peanut, tree nut, fish, shellfish, sesame, mustard, etc. Keep it short and comma-separated. Return ONLY a JSON object mapping the exact dish name to its allergens string.\n\n${listText}`;
        ...
        normalized.forEach(it => {
            result[it.name] = String(map[it.name] || '').trim();
        });
        return result;
    } catch (error) {
